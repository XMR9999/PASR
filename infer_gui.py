...
class SpeechRecognitionApp:
    def __init__(self, window: Tk, args):
        ...
        if not self.use_server:
            # 获取识别器
            self.predictor = PASRPredictor(model_dir=args.model_dir,
        ...
        if not self.use_server:
            ...
            while self.recording:
                ...
                result = self.predictor.predict_stream(audio_data=data, use_punc=args.use_pun, is_itn=self.is_itn,
                                                       is_final=not self.recording, sample_rate=self.sample_rate)
                if result is None: continue
                ## score, text = result['score'], result['text']
                text = result['text']
                self.result_text.delete('1.0', 'end')
                self.result_text.insert(END, f"{text}\n")
            self.predictor.reset_stream_state()
