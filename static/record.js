...
var PASRRecorder = function (stream, url, textResult) {
...
//抛出异常
PASRRecorder.throwError = function (message) {
    alert(message);
    throw new function () {
...
//是否支持录音
PASRRecorder.canRecording = (navigator.getUserMedia != null);
//获取录音机
PASRRecorder.get = function (callback, url, textarea) {
    if (callback) {
        if (navigator.getUserMedia) {
            navigator.getUserMedia(
                {audio: true} //只启用音频
                , function (stream) {
                    var record = new PASRRecorder(stream, url, textarea);
                    callback(record);
                }
                , function (error) {
                    switch (error.code || error.name) {
                        ...
                        case 'PermissionDeniedError':
                            PASRRecorder.throwError('用户拒绝提供信息。');
                            break;
                        case 'NOT_SUPPORTED_ERROR':
                        case 'NotSupportedError':
                            PASRRecorder.throwError('浏览器不支持硬件设备。');
                            break;
                        case 'MANDATORY_UNSATISFIED_ERROR':
                        case 'MandatoryUnsatisfiedError':
                            PASRRecorder.throwError('无法发现指定的硬件设备。');
                            break;
                        default:
                            PASRRecorder.throwError('无法打开麦克风。异常信息:' + (error.code || error.name));
                            break;
                    }
                });
        } else {
            window.alert('不是HTTPS协议或者localhost地址，不能使用录音功能！')
            PASRRecorder.throwErr('当前浏览器不支持录音功能。');
            return;
        }
    }
};